...
export const commonValidations = {
  ...
  password: (field: string = "password") =>
    body(field)
      .isLength({
        min: parseInt(process.env.MIN_PASSWORD_LENGTH || "8"),
        max: 128,
      })
      .withMessage(
...
export const validationSets = {
  ...
  changePassword: [
    ...
    commonValidations.password("newPassword").optional(),
  ],
  ...
  createProxy: [
    ...
    commonValidations.port("target_port"),
    body("ssl_enabled")
      .optional()
      .custom((value) => {
        if (typeof value === "boolean") return true;
        if (typeof value === "string") {
          return ["true", "false", "on", "off", "1", "0"].includes(
            value.toLowerCase()
          );
        }
        throw new Error("ssl_enabled must be a boolean value");
      })
      .withMessage("ssl_enabled must be a boolean value"),
  ],
